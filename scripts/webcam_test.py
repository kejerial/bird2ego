...
from bird2ego.contact.hand_detector import FINGERTIPS, HandDetector, ImprovedContactDetector
from bird2ego.egocentric import EgocentricRenderer, EgocentricTransformer
...
class WebcamTestBench:
    ...
    def _init_components(self):
        ...
        if self.skip_frames > 0:
            print(
                f"  Frame skip: YOLO runs every {self.skip_frames + 1} frames (pose/hands every frame)"
            )
    ...
    def _process_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        if self.show_ego:
            self.current_ego_frame = self._compute_egocentric(pose_frame, tracked_objects)
        ...
        if pose_frame is not None:
            ...
            if conf[5] > 0.3 and conf[7] > 0.3 and conf[9] > 0.3:
                ...
                for i, idx in enumerate([5, 7, 9]):
                    ...
                    y_body = (
                        np.dot(rel, body_forward) / shoulder_width
                    )  # Forward/back (depth estimate)
    ...
    def _render_egocentric_window(self) -> np.ndarray:
        ...
        if self.ego_recording:
            ...
            cv2.putText(
                ego_view,
                f"REC {self.ego_frames_recorded}",
    ...
    def _start_ego_recording(self):
        ...
        self.ego_writer = cv2.VideoWriter(self.ego_record_path, fourcc, self.fps, (960, 720))
    ...
    def _stop_ego_recording(self):
        ...
        self.ego_recording = False
        print(
            f"Recording stopped: {self.ego_frames_recorded} frames saved to {self.ego_record_path}"
        )
        self.ego_record_path = None
        self.ego_frames_recorded = 0
    ...
    def _draw_pose(self, frame: np.ndarray, pose_frame) -> np.ndarray:
        ...
        for joint1_name, joint2_name in SKELETON_CONNECTIONS:
            ...
            if (
                ...
                and conf[idx1] > conf1_thresh
                and conf[idx2] > conf2_thresh
            ):
    ...
    def _draw_objects(
        ...
        for track_id, det in tracked_objects:
            ...
            cv2.rectangle(
                frame,
                (pt1[0], pt1[1] - label_h - 5),
                (pt1[0] + label_w, pt1[1]),
                COLORS["bbox_object"],
                -1,
            )
            cv2.putText(
                frame, label, (pt1[0], pt1[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLORS["text"], 1
            )
    ...
    def _draw_contacts(
        ...
        objects_for_contact = [
            (track_id, det.bbox_xyxy, det.class_name) for track_id, det in tracked_objects
        ]
        ...
        for contact_state in self.current_contacts:
            ...
            if contact_state.is_grabbing:
                ...
                cv2.putText(
                    frame,
                    f"GRABBING! ({contact_state.grab_confidence:.0%})",
    ...
    def _draw_info(self, frame: np.ndarray) -> np.ndarray:
        ...
        fps_color = (
            (0, 255, 0)
            if self.fps_display >= 20
            else ((0, 255, 255) if self.fps_display >= 10 else (0, 0, 255))
        )
        cv2.putText(
            frame,
        ...
        cv2.putText(
            ...
            cv2.FONT_HERSHEY_SIMPLEX,
            0.45,
            (0, 255, 0) if self.show_pose else (100, 100, 100),
            1,
        )
        y += 20
        ...
        cv2.putText(
            ...
            cv2.FONT_HERSHEY_SIMPLEX,
            0.45,
            (0, 255, 0) if self.show_objects else (100, 100, 100),
            1,
        )
        y += 20
        ...
        hand_color = (
            (0, 255, 255)
            if num_detected > 0
            else ((0, 165, 255) if num_predicted > 0 else (100, 100, 100))
        )
        ...
        if num_hands > 0:
            hand_text += " [3D]"
        cv2.putText(frame, hand_text, (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.45, hand_color, 1)
        y += 20
        ...
        contact_color = (
            (0, 0, 255) if grabbing else ((0, 165, 255) if num_contacts > 0 else (100, 100, 100))
        )
        contact_text = (
            "GRABBING"
            if grabbing
            else (f"Contacts: {num_contacts}" if num_contacts else "No contact")
        )
        cv2.putText(frame, contact_text, (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.45, contact_color, 1)
...
""",
    ...
    parser.add_argument("--no-objects", action="store_true", help="Disable object visualization")
    parser.add_argument("--no-contacts", action="store_true", help="Disable contact visualization")
    parser.add_argument("--show-3d", action="store_true", help="Show 3D pose view")
    parser.add_argument(
        "--show-ego", action="store_true", help="Show egocentric (first-person) view overlay"
    )
    parser.add_argument(
        "--stub", action="store_true", help="Use stub/fake backends instead of real detection"
    )

    # Performance options
    parser.add_argument(
        "--fast", action="store_true", help="Fast mode: lite models + skip frames (best FPS)"
    )
    parser.add_argument("--lite", action="store_true", help="Use lighter/faster model variants")
    parser.add_argument(
        "--skip",
        type=int,
        default=0,
