...
class EgocentricFrame:
    ...
    objects_ego: Dict[int, np.ndarray] = field(default_factory=dict)  # obj_id -> (3,) center position
            ...
            "frames": [
                {
                    ...
                    "left_hand_ego": f.left_hand_ego.tolist() if f.left_hand_ego is not None else None,
                    "right_hand_ego": f.right_hand_ego.tolist() if f.right_hand_ego is not None else None,
                    ...
                    "right_arm_ego": f.right_arm_ego.tolist() if f.right_arm_ego is not None else None,
...
class EgocentricTransformer:
    ...
    def estimate_head_pose(
        ...
        if valid_left_eye and valid_right_eye:
            ...
            if valid_left_shoulder and valid_right_shoulder:
                ...
                return (np.zeros(3), np.array([0, 0, 1]), np.array([0, -1, 0]), 
                       np.array([1, 0, 0]), 0.0)
    ...
    def process_frame(
        ...
        if pose_3d is not None and pose_conf is not None:
            head_pos, forward, up, right, head_conf = self.estimate_head_pose(
                pose_3d, pose_conf
            )
        ...
        if left_hand_3d is not None and left_hand_conf > 0.3:
            left_hand_ego = self.transform_to_egocentric(
        ...
        if pose_3d is not None and pose_conf is not None:
            ...
            if (pose_conf[l_shoulder_idx] > 0.3 and pose_conf[l_elbow_idx] > 0.3 
                and pose_conf[l_wrist_idx] > 0.3):
                left_arm = np.stack([
                    pose_3d[l_shoulder_idx],
                    pose_3d[l_elbow_idx],
                    pose_3d[l_wrist_idx]
                ])
                left_arm_ego = self.transform_to_egocentric(
                    left_arm, head_pos, forward, up, right
                )
            ...
            if (pose_conf[r_shoulder_idx] > 0.3 and pose_conf[r_elbow_idx] > 0.3 
                and pose_conf[r_wrist_idx] > 0.3):
                right_arm = np.stack([
                    pose_3d[r_shoulder_idx],
                    pose_3d[r_elbow_idx],
                    pose_3d[r_wrist_idx]
                ])
