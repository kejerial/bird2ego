...
class EgocentricRenderer:
    ...
    def render(
        ...
        if ego_frame.left_hand_ego is not None:
            self._draw_hand_3d(
                canvas, ego_frame.left_hand_ego, ego_frame.left_hand_pinching, "left"
            )
        if ego_frame.right_hand_ego is not None:
            self._draw_hand_3d(
                canvas, ego_frame.right_hand_ego, ego_frame.right_hand_pinching, "right"
            )
    ...
    def _draw_arm_3d(self, canvas: np.ndarray, arm_ego: np.ndarray, side: str):
        ...
        # Draw joints
        for i, (color_key, size) in enumerate([("shoulder", 12), ("elbow", 10), ("wrist", 8)]):
            if valid[i]:
                ...
                if self._in_bounds(pt):
                    ...
                    # Highlight
                    cv2.circle(canvas, pt, radius - 2, tuple(min(255, c + 30) for c in color), -1)
    ...
    def _draw_hand_3d(self, canvas: np.ndarray, hand_ego: np.ndarray, is_pinching: bool, side: str):
        ...
        for i, idx in enumerate(self.fingertip_indices):
            if valid[idx]:
                ...
                if self._in_bounds(pt):
                    ...
                    # Highlight
                    cv2.circle(canvas, pt, radius - 2, tuple(min(255, c + 40) for c in color), -1)
        ...
        if is_pinching and valid[4] and valid[8]:
            ...
            if self._in_bounds(thumb_pt) and self._in_bounds(index_pt):
                center = ((thumb_pt[0] + index_pt[0]) // 2, (thumb_pt[1] + index_pt[1]) // 2)
    ...
    def _draw_3d_box(
        ...
        overlay = canvas.copy()
        cv2.rectangle(overlay, (x - half, y - half), (x + half, y + half), color, -1)
        alpha = 0.3
    ...
    def _draw_hud(self, canvas: np.ndarray, ego_frame: EgocentricFrame):
        ...
        for hand, is_pinching, conf in [
            ...
            if conf > 0:
                ...
                cv2.putText(
                    canvas, f"{hand}: {status}", (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1
                )
                y += 20
...
def render_egocentric_overlay(
    ...
    cv2.rectangle(
        output, (x - 2, y - 2), (x + overlay_w + 2, y + overlay_h + 2), (100, 100, 100), 2
    )
    output[y : y + overlay_h, x : x + overlay_w] = ego_small
    cv2.putText(
        output, "Egocentric View", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1
    )
