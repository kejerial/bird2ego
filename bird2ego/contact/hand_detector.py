    ...
    def __post_init__(self):
        if self.landmark_confidence is None:
            ...
            self.landmark_confidence = np.array(
                [float(c) if c is not None else self.confidence for c in self.landmark_confidence],
                dtype=np.float32,
            )
        ...
        if results.hand_landmarks:
            for i, hand_lms in enumerate(results.hand_landmarks):
                ...
                landmark_conf = np.array(
                    [float(getattr(lm, "visibility", 0.9) or 0.9) for lm in hand_lms],
                    dtype=np.float32,
                )
...
class ImprovedContactDetector:
            ...
            for obj_id, bbox, class_name in objects:
                ...
                if contact_info["is_contact"]:
                    ...
                    if contact_info["is_grab"]:
                        ...
                        state.grab_confidence = max(
                            state.grab_confidence, contact_info["grab_confidence"]
                        )
    ...
    def _check_hand_object_contact(
        ...
        for ft_idx in FINGERTIPS:
            ...
            conf = (
                float(hand.landmark_confidence[ft_idx])
                if (
                    hand.landmark_confidence is not None
                    and hand.landmark_confidence[ft_idx] is not None
                )
                else 1.0
            )
        ...
        if fingertips_near >= 2 or fingertips_inside >= 2 or (palm_inside and fingertips_near >= 1):
            ...
            grab_conf = (
                fingertip_score * 0.4 + pinch_score * 0.3 + palm_score * 0.3
